...
class Grid:
    def __init__(self, line_groups: list[list[point]], sand_src: list[point]):
        ...
        self.abyss_y = reduce(max, [r.y for r in rocks]) + 1
        # noinspection PyTypeChecker
        self.max_x = reduce(max, [r.x for r in rocks]) + 1

        for y in range(self.abyss_y):
            row = []
    ...
    def flow_grain(self, grain: point) -> tuple[point, FlowResult]:
        ...
        for position in positions_to_consider:
            if position.y >= self.abyss_y:
                return position, FlowResult.ABYSS_REACHED

            if self.grid[position.y][position.x] not in flow_blockers:
